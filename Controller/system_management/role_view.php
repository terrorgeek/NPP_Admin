...
$out_find_level = $db->fetch_array($res_find_level);
	...
	while($root_page = $db->fetch_array($res_find_root_page))
	{
		...
		{
			$page_out .= "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
			$page_out .= "<input type=\"checkbox\" name=\"".$find_chhild_page['page_id']."\" ".check_status($user_id,$find_chhild_page['page_id'],$db)." disabled=\"disabled\">".$name[$find_chhild_page['page_id']]."<br>";
		}
	}
	$db->close();
	$page_out .= "<br>";
	$page_out .="<input type=button name=\"test\" class=\"button\" onclick=\"javascript:role_back();\" value=\"返回\">";
	$page_out .= "</form>";
...
$smarty->display("system_management/role_view.htm");
?>